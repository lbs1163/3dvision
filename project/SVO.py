...
def SVO(sequence, startFrame, endFrame, useSIFT=False, useRansac=False, showLiveTrajectory=False):
    ...
    for frm in range(startFrame+1, endFrame+1):
        ...
        if True:#pointDiffSum >  (trackPoints1_KLT_L.shape[0] * 0.05):
            ...
            if len(translation) != 0:
                translation.append(np.matmul(rotation, translationArray))
            else:
                translation.append(translationArray)

            rotation.append(Rmat)
        ##########################################
